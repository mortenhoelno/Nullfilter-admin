...
Dokument lastes opp i admin → Supabase Storage

syncMissingFiles() → registrerer i documents

chunker.js → splitter i chunks, lagrer i rag_chunks med embeddings

Chatbot → rag/search.js henter relevante chunks (semantic search via vector)

GPT får kontekst + brukerinput → svar lagres (foreløpig ikke i DB)

3.2 Chatter (planlagt)

Bruker starter → chat_sessions opprettes

Meldinger → lagres i chat_messages

Kontekstkobling → message_context_links binder meldinger til chunks

Preferanser → session_settings

Langsiktig minne → user_memory
...
Opprette tabeller for logging (chat_sessions, chat_messages, message_context_links, session_settings, user_memory)

Stramme inn RLS på documents

Standardisere embeddings → vector(1536) overalt

Fjerne gamle tabeller etter at vi er trygge (evt. beholde views for kompatibilitet)

Lage views / dashboard for status og analyser
