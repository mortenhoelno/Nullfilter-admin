export const config = {
  runtime: "edge", // NB: vi tester Vercel edge runtime direkte
};
...
export default async function handler(req) {
  ...
  const userPrompt = body?.q || "Hei, test stream!";
  ...
  if (!resp.ok || !resp.body) {
    return new Response(`OpenAI error: ${resp.status} ${await resp.text()}`, {
      status: 500,
    });
  }
